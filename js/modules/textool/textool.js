...
layui.define(['jquery'], function(exports) {
  var $ = layui.$, baseClassName = 'layext-text-tool', extClassName = 'layext-textool-pane',
    tips, selector, style, alignRight = true, alignClass, nodes = [], tools = {
    ...
    init: function(custom) {
      ...
      selector = $.trim(_this.options.eleId) === '' ? '.' + baseClassName : '#' + _this.options.eleId;
    ...
    initStyle: function(_this) {
      ...
      style = ['<style type="text/css">',
        ...
        selector, ' { position: relative; z-index: ', _this.options.zIndex + 1, '; }',
        selector, ' + .', extClassName, ' { position: relative; z-index: ', _this.options.zIndex, '; margin-top: -2px; display: block; outline: none; }', 
        selector, ' + .', extClassName, ' * { color: ', (_this.options.color || '#666666'), '; }',
        selector, ' + .', extClassName, ' a > i { font-size: 12px!important; }',
        selector, ' + .', extClassName, ' a { padding: 0 3px; cursor: pointer; }',
        selector, ' + .', extClassName, ' a:active { background-color: #E2E2E2; opacity: 0.8; }',
        selector, ' + .', extClassName, ' .', tools.lengthClass, ' * { font-family: Consolas, sans-serif; }',
        selector, ' + .', extClassName, ' .', tools.lengthClass, ' { display: inline-block; border-width: 0 1px; }', /* border: 1px solid #F6F6F6; */
        selector, ' + .', extClassName, ' .', tools.lengthClass, ' * { font-family: Consolas, sans-serif; }',
        selector, ' + .', extClassName, ' .', tools.lengthOverClass, ' { color: #FF5722; }',
        selector, ' + .', extClassName, ' .', tools.countClass, ', ', selector, ' + .', extClassName, ' .', tools.maxClass, ' { display: inline-block; min-width: 26px; height: 16px; line-height: 18px; }',
        selector, ' + .', extClassName, ' > .layui-badge { overflow: hidden; border-color: ', (_this.options.borderColor || '#E6E6E6'), '; background-color: ', (_this.options.bgColor || '#FFFFFF'), '; }',
        selector, ' + .', extClassName, ' .', tools.maxClass, ' { opacity: 0.9; }',
        selector, ' + .', extClassName, '-r { text-align: right; }',
        selector, ' + .', extClassName, '-l { text-align: left; }',
        '</style>'].join('');
    ...
    /** 添加文本工具 */
    addTextool: function(_this, $target) {
      ...
      _this.display(_this, $target, $extPane);
    },
    display: function(_this, $target, $extPane) {
      $extPane.fadeIn();
    },
    /** 复制文本 */
    copyText: function(_this, $target) {
      if (!$target) {
        return false;
      }
    ...
    /** 设置内容长度 */
    setValLength: function(_this, $target) {
    ...
    /** 设置工具条事件 */
    setEvent: function(_this, $target, $extPane) {
      ...
      $extPane.on('click', 'a', function(e) {
        ...
        if($this.hasClass(tools.hideClass)) {
          ...
          tips === tools.laytips && _this.showTip(_this, $this, $this.attr(tips));
        }
    ...
    /** 获取工具条节点 */
    getToolsNode: function(_this, $target) {
      ...
      // 处理工具条节点
      nodes = ['<span class="', extClassName, ' ', alignClass, ' ', $.trim(_this.options.className), 
    ...
    /** 初始化吸附提示 */
    initTips: function(_this) {
      $('[' + tools.laytips + ']').each(function(i, n) {
        var $target = $(n);
        if ($.trim($target.attr(tips)) !== '') {
          $target.hover(function() {
            _this.showTip(_this, $target, $target.attr(tips));
          }, _this.hideTip);
        }
      });
    },
    /** 显示吸附提示 */
    showTip: function(_this, $target, msg) {
      _this.hideTip();
